...
- attaches a policy-defined authorization envelope (scopes, resources, groups, optional email/name)
...
- Enforcing service-session constraints (resource presence, renewal policy, etc.)
...
- `userinfo.resources` (resource list)
...
- Requested `resource` values must be a subset of configured allowed resources
- If `scope` is omitted, `default_scopes` is used (if present)
- If `resource` is omitted, configured `resources` are used

At runtime (e.g., `/session` access):

- Service sessions are expected to include `resources`
- Misconfigured/missing `resources` can be treated as a hard deny (403)
...
`client_id`) to authorization policy. A _binding_ should uniquely identify a principal, its allowed scopes/resources, and issuance policy.
      ...
      "bindings": [
        {
          ...
          "resources":        ["urn:deriva:rest:service:all"],
          "groups":           ["etl-writers"],
...
- If `resources` is empty/missing for a binding, issuance fails (treated as misconfiguration).
...
- `userinfo.resources`: configured resources
...
curl -sS -X POST https://<credenza>/authn/service/token \
  ...
  -d resource="urn:deriva:rest:service:all"
```
...
If you omit `resource`, Credenza defaults to:
...
- Set resources narrowly wherever possible. Resource is the primary mechanism to prevent token reuse across services.
